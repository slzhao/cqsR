...
proteinExpression  %>% spread(Sample, nLog2)

#oder by abs value of rev(nLog2) so that distinct will keep the first (largest) row
#temp=abs(proteinExpression$nLog2)
#proteinExpression[rev(order(temp))[1:20],] %>% distinct(pID,Sample, .keep_all = TRUE) %>% spread(Sample, nLog2)
